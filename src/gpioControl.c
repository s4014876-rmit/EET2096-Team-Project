...
void gpio_configureGPIO(GPIO_Config * inputGPIO)
{
	...
	inputGPIO->port->MODER &= (unsigned int)(~(0x03 << (inputGPIO->pin * 2)));				// Mode Register
	inputGPIO->port->OTYPER &= (unsigned int)(~(0x01 << inputGPIO->pin));						// Output Type Register
	inputGPIO->port->OSPEEDR &= (unsigned int)(~(0x03 << (inputGPIO->pin * 2)));			// Output Speed Register
	inputGPIO->port->PUPDR &= (unsigned int)(~(0x03 << (inputGPIO->pin * 2)));				// Pull-up / Pull-down register
	
	// Set the configuration of the pins (defined in enums, so no checking is necessary)
	inputGPIO->port->MODER |= (unsigned int)(inputGPIO->mode << (inputGPIO->pin * 2));			// Set the mode register		
	inputGPIO->port->OTYPER |= (unsigned int)(inputGPIO->outputType << inputGPIO->pin);						// Output Type Register
	inputGPIO->port->OSPEEDR |= (unsigned int)(inputGPIO->speed << (inputGPIO->pin * 2));					// Output Speed Register
	inputGPIO->port->PUPDR |= (unsigned int)(inputGPIO->pullUpDown << (inputGPIO->pin * 2));				// Pull-up / Pull-down register
}
